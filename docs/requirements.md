...
| ユーザー | 説明 |
|----------|------|
| 一般従業員 | 従業員番号でログイン。打刻記録、メンバー登録を利用 |
| admin | 「admin」でログイン。全機能を利用可能 |
...
- ログイン状態はHTTPOnly Cookieで30日間保持する
...
- **iPhone/NFC非対応デバイスの場合:**
  - 自動的にQRコードスキャナーモードに切り替える
  - カメラでQRコードを読み取り、UIDを取得して従業員を照合する
...
- 初期状態ではメンバーは表示されない
- 「閲覧メンバー追加」ボタンで登録済み従業員のチェックリストを表示し、チェックを入れて一括追加する
  - 追加情報はviewing_membersテーブルに永続化する
  - 既に追加済みのメンバーはリストに表示されない
  - 複数メンバーの同時追加が可能
- 追加済みメンバーの打刻一覧をテーブル形式で表示する
...
- 打刻記録画面でNFC非対応デバイス（iPhone等）の場合、自動的にQRスキャナーに切り替える
